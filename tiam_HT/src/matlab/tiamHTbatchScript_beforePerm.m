function tiamHTbatchScript_beforePerm(batchSetupCell)
...
for job=1:length(batchSetupCell)
    ...
    % image order
    startimg = jobSetup{6};
    startimg_flur1 = jobSetup{7};
    startimg_flur2 = jobSetup{8};
    startimg_flur3 = jobSetup{9};
    startimg_flur4 = jobSetup{10}; % reporter not included before permeablizatn
    % detectParams
    ...
    cellData=convertUnits_beforePerm(cellData,numUmPerPix_convert);
    ...
    delFields={'cellMask','dilCellMask','bgMask','flur1_img','flur1_mask','flur2_img','flur2_mask','flur3_img','flur3_mask'}; % no flur4
    ...
    report=create96wellReport_beforePerm(cellData);
